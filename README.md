...
Então, dependendo do modo de execução do Spark (client mode ou cluster mode) os componentes do programa Spark irão se conectar aos componentes do cluster manager. No cluster mode, o cluster manager irá possuir o mapeamento do master node, onde possui seu próprio driver, e dos worker nodes. Quando a aplicação Spark é submetida, o driver manager aloca recursos para executar o driver program em um dos worker nodes e só depois irá alocar recursos para executar as tarefas nos outros worker nodes. Abaixo, na figura que representa o cluster mode, o driver program roda no primeiro node enquanto as tarefas foram distribuídas nos demais nodes.

No modo cliente mode, a figura do cluster manager continua igual, ainda possui a responsabilidade de mapear os worker nodes do cluster, porém, nesse modo de execução a aplicação Spark é submetida através de uma máquina fora do cluster. Essa máquina é conhecida como add node. E nesse caso, o cluster manager aloca recursos para executar uma tarefa por node mas não alocou recursos para executar o driver program. Nesse tipo de execução, o driver program irá rodar na mesma máquina em que a aplicação foi iniciada.

Enquanto a aplicação estiver executando, os worker nodes podem comunicar-se na troca de mensagens e dados, como por exemplo nas operações de shift onde um relacionamento wide é encontrado. Quando a execução termina, o driver do cluster manager recebe notificação de sucesso ou falha. 
